...
const getSyncedEvents = (items) => items
  .filter(({success}) => success).map(({payload}) => payload.point);
...
export default class Provider {
  ...
  createEvent(event) {
    ...
    const fakeNewEvent = EventModel.parseEvent(Object.assign({}, event.toRAW(), {id: fakeNewEventId}));
  ...
  updateEvent(id, event) {
    ...
    const fakeUpdatedEvent = EventModel.parseEvent(Object.assign({}, event.toRAW(), {id}));
